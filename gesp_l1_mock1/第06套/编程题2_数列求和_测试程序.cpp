// =============================================================
// GESP C++ 一级 模拟卷 第 06 套 - 编程题 2《数列求和》评测程序
//   g++ 你的代码.cpp -o my_answer
//   g++ 编程题2_数列求和_测试程序.cpp -o judge2
//   ./judge2 ./my_answer
...
static string solve(long long n){ double s=0; for(long long i=1;i<=n;i++) s+=1.0/i; char b[64]; sprintf(b,"%.2f",s); return string(b); }
int main(int argc,char**argv){
    if(argc<2){printf("用法: %s <考生可执行文件路径>\n",argv[0]);return 1;}
    string exe=argv[1];
    vector<long long> ins={1,3,2,5,10,100,1000,50,7,20};
