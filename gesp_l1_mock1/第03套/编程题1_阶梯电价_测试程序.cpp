// =============================================================
// GESP C++ 一级 模拟卷 第 03 套 - 编程题 1《阶梯电价》评测程序
// -------------------------------------------------------------
//   g++ 你的代码.cpp -o my_answer
//   g++ 第03套_编程题1_阶梯电价_测试程序.cpp -o judge
...
static string runProgram(const string& exe,const string& input){
    string in="._j31in.txt",out="._j31out.txt";
...
static string solve(long long x){
    double fee = (x<=200) ? x*0.5 : 200*0.5 + (x-200)*0.8;
    char buf[64]; sprintf(buf,"%.2f",fee); return string(buf);
}

int main(int argc,char** argv){
    if(argc<2){printf("用法: %s <考生可执行文件路径>\n",argv[0]);return 1;}
    string exe=argv[1];
    vector<long long> ins={100,300,200,0,201,1000,500,199,100000,250};
    vector<Case> cases;
